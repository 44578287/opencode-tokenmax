...
import { TokenMaxCompletionGate } from "@/tokenmax/completion-gate"
import type { RunRequirement } from "@/tokenmax/completion-gate"

const { detectEarlyStop, EarlyStopTracker, evaluateStop } = TokenMaxCompletionGate
...
describe("evaluateStop", () => {
  ...
  it("3.7 boundary: evaluateStop() has no textPreview/finish_reason parameter at all -- model prose cannot reach it", () => {
...
describe("detectEarlyStop", () => {
  ...
  it("catches the Chinese phrasing called out in TOKENMAX-RELIABILITY.md", () => {
    expect(
