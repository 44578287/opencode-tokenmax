export * as TokenMaxCompletionGate from "./completion-gate"

// R2.5 -- Reliability + Event Runtime (docs/TOKENMAX-ROADMAP.md), regression
// 3.7 in docs/TOKENMAX-RELIABILITY.md ("model early-stop"). Ported from the
// frozen R2.5 prototype (packages/tokenmax on
// claude/event-driven-execution-model-ge98zn, see docs/TOKENMAX-DECISIONS.md
// D-003) after checking the D-001 reuse criteria: this module is pure,
// self-contained (its only external type dependency, RunTerminalState, is
// inlined below rather than pulling in the prototype's operation-manager.ts,
// which D-011 already decided NOT to port), fully test-covered, and there is
// no existing upstream OpenCode primitive that does this -- OpenCode's
// SessionPrompt loop trusts a model's own `finish_reason` as-is, with no
// requirement-based gate. Not a reimplementation of anything upstream
// already owns.
//
...
// progress, only explicit `RunRequirement`s supplied by the caller (pending
// DAG nodes, workers promised but not started, verifiers not yet run, the
// user's objective not yet satisfied).
//
...
// human".
//
// Not yet wired into the live session loop -- see TOKENMAX-ROADMAP.md's R2.5
// section for what's landed vs. not. This slice is the pure decision logic,
// tested in isolation, matching how router.ts/telemetry.ts landed ahead of
// their own downstream wiring.

/** The only four terminal outcomes for a whole Run. `finish_reason: "stop"`
 * from a model is NOT one of these -- it is an input signal that the
 ...
 * text. Extend this list rather than adding an escape hatch for "the model
 * said so".
 */
...
// Chinese phrasing called out explicitly in docs/TOKENMAX-RELIABILITY.md 3.7.
 ...
 * ladder from TOKENMAX-RELIABILITY.md 3.7: request an explicit tool action,
 * then a stronger instruction, then fall back to a different root model,
 * then give up with an explicit FAILED -- never an infinite self-retry loop.
 *
 * `record()` returns a RECOMMENDATION, not a state transition: this class
 * holds no reference to any Run/session state and cannot itself fail or
 * complete anything. Even `"EXPLICIT_FAILURE"` is advisory -- the caller
 * decides whether and how to actually fail the relevant run. This keeps
 * early-stop detection strictly downstream of, and never a substitute for,
 * the authoritative requirement sources in `evaluateStop()`.
 */
